...
class Map(object):
    ...
    def __init__(self, mapname, authorname, mapsize, tileset, tilesize, backgrounds):
        ...
        self.bgTiles = [0 for i in xrange(len(backgrounds))]
    ...
    def _createBackgrounds(self):
        ...
        for b in self.backgrounds:
            ...
            bsrc = rggResource.crm.translateFile(b, rggResource.RESOURCE_IMAGE)
            infograb = QtGui.QPixmap(bsrc)
            temptile = (rggTile.tile(
                ((i % 2) * 1000, int(i/2) * 1000),
                (infograb.width(), infograb.height()), (1000, 1000), 0, 0, bsrc))
            temptile.setHidden(self.hidden)
            self.bgTiles[j] = temptile
            j += 1
    ...
    def load(obj, dumpmode=False):
        ...
        map = Map(
            ...
            loadCoordinates('Map.mapsize', obj.get('mapsize'), length=2, min=0, max=65535),
            loadString('Map.tileset', obj.get('tileset')),
            loadCoordinates('Map.tilesize', obj.get('tilesize'), length=2, min=1, max=65535),
            loadArray('Map.backgrounds', obj.get('backgrounds')))
